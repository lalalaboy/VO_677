...
# Priority: conda environment (for glog) > system paths (for gdal/tiff) > demo folder
conda_prefix = os.environ.get('CONDA_PREFIX', '')
demo_dir = os.path.dirname(os.path.abspath(__file__))
if conda_prefix:
    new_ld_path = f'{conda_prefix}/lib:/usr/lib/x86_64-linux-gnu:/usr/local/lib:{demo_dir}'
else:
    new_ld_path = f'/usr/lib/x86_64-linux-gnu:/usr/local/lib:{demo_dir}'

existing_ld_path = os.environ.get('LD_LIBRARY_PATH', '')
if existing_ld_path:
    # Prepend our paths, avoiding duplicates
    existing_parts = existing_ld_path.split(':')
    new_parts = new_ld_path.split(':')
    filtered_existing = [p for p in existing_parts if p not in new_parts and p]
    final_ld_path = new_ld_path + (':' + ':'.join(filtered_existing) if filtered_existing else '')
else:
    final_ld_path = new_ld_path

os.environ['LD_LIBRARY_PATH'] = final_ld_path

# Force use system libtiff to avoid conflict with conda's libtiff (required by libgdal)
